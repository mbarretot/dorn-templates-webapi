...
- An interactive OpenAPI UI (Scalar) at `/scalar` in Development, served alongside the existing `/openapi/v1.json` document. `Microsoft.AspNetCore.OpenApi` generates the document; `Scalar.AspNetCore` only renders it, so nothing about the document itself changes.
- `BlobStorage` template parameter (choice: `manual` default, `database`, `filesystem`). `manual` generates nothing and the solution is unchanged. `database` and `filesystem` add an `IBlobStore` port in the Application layer (save, open, exists, delete, with content type and size) plus `BlobRules`, which validates blob names before any store uses them. `database` keeps blobs in a `Blobs` table for both ORMs and every database: an `AddBlobs` migration for EF Core (SQLite, SQL Server, PostgreSQL) or a schema bootstrap in `DapperContext` for Dapper. `filesystem` stores files under `BlobStorage:FileSystem:RootPath` (default `App_Data/blobs`) and refuses names that could escape the root; Docker Compose mounts a named volume for it. Database blobs are buffered in memory and bounded by the provider's value size limit, so they suit small files; no endpoints are generated. Works with every `Orm`, `DatabaseProvider`, `Orchestrator`, and `Auth` choice, with and without `IncludeSample`, and with `IncludeLocalization`.
